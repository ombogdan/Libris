...
create index if not exists book_listings_feed_idx
on public.book_listings (status, is_deleted, created_at desc)
where status = 'active' and not is_deleted;

create index if not exists book_listings_category_idx
on public.book_listings (category)
where status = 'active' and not is_deleted;
