-- search_book_listings previously required an exact (case/whitespace
-- insensitive) match on city, so a typo or a different spelling than what
-- a seller typed silently returned zero results. Matching it the same way
-- p_query already matches title/author/city (ILIKE substring) means typing
-- part of a city name is enough — no need to guess the exact spelling or
-- pick from a suggestion list first. Signature is unchanged from the
-- previous version, so this is a plain create-or-replace.
  ...
  with scored as (
    ...
    where
      ...
      and (
        ...
        or listing.city ilike '%' || normalized_city || '%'
      )
